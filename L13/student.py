...
def db_setup():
    ...
    try:
        con = connect("sms.db")
        cursor = con.cursor()
        cursor.execute("create table if not exists student(rno int primary key, name text)")
        con.commit()
...
def save():
    try:
        ...
        con = connect("sms.db")
        cursor = con.cursor()
        cursor.execute("insert into student values(?, ?)", (rno, name))
        con.commit()
...
def view():
    ...
    try:
        ...
        for d in data:
            vw_st_data.insert(END, f"Rno = {d[0]}   Name = {d[1]}\n")
    except Exception as e:
        showerror("Error", e)
    finally:
        if 'con' in locals():
